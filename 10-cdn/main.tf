resource "aws_cloudfront_distribution" "bhargavdevops" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.bhargavdevops.domain_name
        zone_id = aws_cloudfront_distribution.bhargavdevops.hosted_zone_id
      }
    }
  ]
}